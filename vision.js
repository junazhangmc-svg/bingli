...
"",
"【不同类型的报告，重点完全不同】",
"检验单（生化、血脂、血常规、甲功等）：主体是表格，重点在 observations，逐行提取。",
"影像与功能检查（超声、CT、MRI、X线、心电图、胃肠镜、病理）：",
"  这类报告通常没有数值表格，observations 可以是空数组，这很正常，不是失败。",
"  重点是把「检查所见」原文完整抄进 findings，把「诊断意见 / 结论 / 检查提示」抄进 impression。",
"  报告里如果有带数字的测量（如内膜厚度 8mm、结节 1.2×0.8cm、颈动脉狭窄 30%、",
"  钙化积分 270），把每一个都作为一条 observation 提取出来，itemName 用它测量的东西。",
"门诊 / 住院记录：重点是 clinicalDiagnosis、findings（病史与查体）、recommendation。",
"体检报告：既有表格也有各科小结，两者都要 —— 表格进 observations，小结进 impression。",
"",
"【逐行提取，绝对不要跨行取值】",
...
function visionWarnBlock(d){
  ...
  var w = d.warnings || {};
  /* 「没读出检查项目」只有对检验单才算失败。
     影像、心电、内镜、病理这类报告本来就没有数值表格，
     它们的内容全在「检查所见」和「结论」里 —— 只要那两处有东西，就算读到了。
     以前一律按 0 项报失败，等于告诉用户「影像报告识别不了」，那是我判错了。 */
  var hasText = !!(d.findings || d.impression || d.dx || d.recommendation);
  var tabular = d.type === "laboratory" || d.type === "checkup";
  var empty = d.obs.length === 0 && !(hasText && !tabular);
  ...
  if (empty) {
    ...
    if (!d.obs.length) {
      /* 影像、内镜、病理这类报告没有数值表格，重点在文字上 */
      h += '<p class="tiny-note">这是一份没有数值表格的报告，' +
           '内容都在下面的<b>结论</b>和<b>检查所见</b>里。' +
           '请对着原件核一遍 —— 尤其是结论那一段，' +
           '模型偶尔会漏掉「建议随访」「必要时复查」这类关键的后半句。</p>';
    } else {
